...
public class BookFlightAction(ILogger<BookCarAction> logger, IPublishEndpoint publishEndpoint) : ActionBase<BookFlightMessage, CancelBookFlightMessage>
{
    ...
    {
        await publishEndpoint.Publish(new ReplyMessage(message.BookingTravelId, EventType.BookFlightCompleted));
    ...
    {
        await publishEndpoint.Publish(new ReplyMessage(message.BookingTravelId, EventType.BookFlightCompensated));
